...
function validateEmail(id) {
    ...
    if (email.length === 0) {
        errorDiv.querySelector(".paragraph-20").textContent = "Representative email is required.";
        gsap.timeline()
    ...
    if (!email.match(emailPattern)) {
        errorDiv.querySelector(".paragraph-20").textContent = "Invalid Email Address.";
        gsap.timeline()
            .to(errorDiv, { display: "flex" })
            .to(errorDiv.children, { display: "flex" }, "<");
        $(formControl).addClass("has-error");
...
const validateWalletAddress = (id) => {
    ...
    if (walletAddress.length === 0) {
        errorDiv.querySelector(".paragraph-20").textContent = "Wallet Address is required.";
        gsap.timeline()
            .to(errorDiv, { display: "flex" })
            .to(errorDiv.children, { display: "flex" }, "<");
        $(formControl).addClass("has-error");
    ...
    if (!walletAddress.match(walletAddressPattern)) {
        errorDiv.querySelector(".paragraph-20").textContent = "Invalid Wallet Address.";
        gsap.timeline()
            .to(errorDiv, { display: "flex" })
            .to(errorDiv.children, { display: "flex" }, "<");
        $(formControl).addClass("has-error");
